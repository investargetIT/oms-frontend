...
import { Button, Card, Col, Form, message, Modal, Row, Space, Tabs, Tag, Upload } from 'antd';
...
const Add: React.FC<AddProps> = () => {
  ...
  useEffect(() => {
    if (isRead) {
      queryApplyDetail({ sid: query?.sid }).then((res: any) => {
        if (res?.errCode === 200) {
          const newData = {
            ...res.data,
            scrapApply: {
  ...
  const submit = async (values: any) => {
    ...
    //   values.invoiceMobile.trim(),
    //   values.invoiceTel.trim(),
    // );
      ...
      >
        ...
        <div className="editContentCol minHeight">
          <Tabs
            ...
            size="large"
            onChange={tabChange}
            className={classNames('cust-tab', { hideTab: isRead ? false : true })}
          >
